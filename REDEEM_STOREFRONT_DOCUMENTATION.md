...
| v1.5 | 2026-04-24 | **Performance, Accessibility & UI Fixes**: Resolved browser preload warnings, added form attributes for autofill, enhanced Privacy Policy with an interactive copy-to-clipboard email, fixed CLS layout shifts on initial load via default RTL and stable scrollbar, fixed z-index overlapping issues in the mobile slider, implemented native lazy loading with animated skeleton placeholders, added missing aria-labels to icon buttons, and corrected heading hierarchy for full Lighthouse a11y compliance. |
